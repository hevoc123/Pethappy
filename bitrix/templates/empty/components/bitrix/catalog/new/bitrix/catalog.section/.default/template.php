...
$this->SetViewTarget("detailtext");//дальше контент который буферизируется
?>
	<div class="grey-bg">
		<div class="inner">
	...
	<li class="item-card <?if(is_array($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):?>withoffer<?endif?>" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
		<div class="for-transform<?if($inbasket):?> flipped<?endif;?>">
			...
			<figure class="back">
				...
				<!-- end rating-wrap -->
				<div class="item-name">
					<a data-mdacl="<?=$arElement["ID"]?>" href="<?=str_replace("/categories", "", $arElement['DETAIL_PAGE_URL'])?>" title='<?=$arElement["NAME"]?>' class="truncate-title"><?=$arElement["NAME"]?></a>
				</div>
				<?if(is_array($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):?>
					<?$tmp=Array();?>
					<?foreach($arElement["OFFERS"] as $key=>$arOffer):?>
