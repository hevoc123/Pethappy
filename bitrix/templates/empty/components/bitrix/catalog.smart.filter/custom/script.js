				...
				else if(arItem.VALUES)
				{
					for (i in arItem.VALUES)
					{
						if (arItem.VALUES.hasOwnProperty(i))
						{
							...
							{
								console.log(control);
								var classtemp = control.checked ? "checkbox checked " : "checkbox ";
								control.parentNode.className = ar.DISABLED ? classtemp + 'disabled': classtemp;
							}
						}
					}
				}
		...
		if(!!modef && !!modef_num)
		{
			...
			if(result.FILTER_AJAX_URL && result.COMPONENT_CONTAINER_ID)
			{
				BX.bind(hrefFILTER[0], 'click', function(e)
				{
					...
					BX.ajax.insertToNode(url, $(this.checkbox).closest("label"));
			...
			else
			{
				...
				$(this.checkbox).closest("label").append(modef);
			}
		}
	}
};
