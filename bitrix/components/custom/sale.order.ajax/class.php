...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function initProperties(Order $order, $isPersonTypeChanged)
	{
		...
		{
			//AddMessage2Log("Бля ".json_encode($order->getUserId()), "my_module_id");
	...
	{
		//AddMessage2Log("Произвольный текст сообщения ".$userId, "my_module_id");
	...
	protected function showOrderAction()
	{
		...
		if($arOrder["DELIVERY_ID"]==5)
		{
			...
			if($shop_address==2)
				$toSend["DELIVERY"]="Выбран самовывоз из магазина - м. Ясенево, ул. Голубинская, дом 16, корпус 'Г', павильон 224";
			if($shop_address==6)
				$toSend["DELIVERY"]="Выбран самовывоз из магазина - м. Бульвар Дмитрия Донского, ул. Знаменские садки, д.1Б";
		}
		else
		{
			$toSend["DELIVERY"]="Включая доставку '".$shipment->getDeliveryName()."': ".floatval($arOrder["PRICE_DELIVERY"])." руб.<br /><br />Адрес доставки - ";
			...
			if ($arVals = $db_vals->Fetch())
				$toSend["DELIVERY"].=", дом ".$arVals["VALUE"];
			...
			if ($arVals = $db_vals->Fetch())
				if($arVals["VALUE"]) $toSend["DELIVERY"].=", кор/стр ".$arVals["VALUE"];
			...
			if ($arVals = $db_vals->Fetch())
				if($arVals["VALUE"]) $toSend["DELIVERY"].=", кв/офис ".$arVals["VALUE"];			
		...
		while ($arItems = $dbBasketItems->Fetch())
		{
			...
			$toSend["ORDER_LIST"].=$arItems["NAME"]." - ".$arItems["QUANTITY"]." шт - ".floatval($arItems["PRICE"])." руб<br />";
		}
