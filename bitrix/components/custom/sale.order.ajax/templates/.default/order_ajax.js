...
(function() {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function(parameters)
        {
			...
			this.propGroups = parameters.propsGroup || {};
            this.templateFolder = parameters.templateFolder || '';
            this.basketItemNoPhoto = this.templateFolder + "/images/no_photo.png";
            this.storeDefaultLogo = this.templateFolder + "/images/no_store.png";
            this.deliveryDefaultLogo = this.templateFolder + "/images/logo-default-d.gif";
            this.paySystemDefaultLogo = this.templateFolder + "/images/logo-default-ps.gif";
		...
		sendRefresh: function(timer)
        {		
            var form, action, actionData;
        ...
        reachGoal: function(section, goal)
        {
            ...
            if (useGoals)
            {
                goalId = this.getGoalId(section, goal);
                window['yaCounter' + counter].reachGoal(goalId);
            }
        },

        getGoalId: function(section, goal)
        {
            if (!section || !goal)
                return '';
        ...
        clickOrderSaveAction: function(event)
        {
            ...
            this.reachGoal(actionSection, 'order');
        ...
        modifyExistingBlocks: function()
        {
            var blocks = this.orderBlockNode.querySelectorAll('.bx-soa-section.bx-active'),
                titleNode, i;

            for (i = 0; i < blocks.length; i++)
            {
                titleNode = blocks[i].querySelector('.bx-soa-section-title');
		...
		showAll: function() 
		{
			var sections = this.orderBlockNode.querySelectorAll('div.bx-soa-section.bx-active'), i;
			var active=true;
...
//			this.editActivePickUpBlock(false);
        ...
        editBasketBlock: function(active)
        {
            ...
            //if(!!this.firstLoad) BX.remove(BX.lastChild(this.basketBlockNode)); 
        ...
        editActiveBasketBlock: function(activeNodeMode)
        {
            var node = !!activeNodeMode ? this.basketBlockNode : this.basketHiddenBlockNode,
                basketContent, basketTable, basketLink;
            ...
            else
            {
				...
				basketLink = BX.create('DIV', {
					props: {className: 'bx-soa-basket'},
					html: "<a href='/personal/cart/'>Изменить заказ в корзине</a>",
				});
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('DIV', {
								...
								children: [basketTable,basketLink]
							})
						]
					})
				);
        ...
        editBasketItems: function(basketItemsNode, active)
        {
			...
			if(count > 5) this.showImage=false;
        ...
        createBasketItem: function(basketItemsNode, item, index, active)
        {
			...
			otherColumns.push(BX.create('DIV', {
                    props: {className: 'bx-soa-item-td deleteCol'},
					html: '<span data-id="'+ item.data.PRODUCT_ID +'"></span>',
            }));
        ...
        createBasketItemImg: function(data)
        {
            ...
            {
                url = data.PREVIEW_PICTURE_SRC;
                url2x = data.PREVIEW_PICTURE_SRC_2X;
            }
            else if (data.DETAIL_PICTURE_SRC && data.DETAIL_PICTURE_SRC.length)
            {
                url = data.DETAIL_PICTURE_SRC;
                url2x = data.DETAIL_PICTURE_SRC_2X;
            }
            else
                url = url2x = this.basketItemNoPhoto;

            img = BX.create('DIV', {props: {className: 'bx-soa-item-imgcontainer'}});
            if (this.params.BASKET_IMAGES_SCALING == 'adaptive')
            {
                img.setAttribute('style',
                    'background-image: url(' + url + ');' +
                    'background-image: -webkit-image-set(url(' + url + ') 1x, url(' + url2x + ') 2x)'
                );
            }
            else
        ...
        createBasketItemContent: function(data)
        {
			...
			if (data.CML2_ATTRIBUTES && data.CML2_ATTRIBUTES.length)
				titleHtml =  titleHtml + "<br /><small>" + data.CML2_ATTRIBUTES + "</small>";
        ...
        createBasketItemColumn: function(column, allData, active)
        {
            ...
            {
                var hasImage = allData.data.DETAIL_PICTURE_SRC && allData.data.DETAIL_PICTURE_SRC.length,
                    url = hasImage ? allData.data.DETAIL_PICTURE_SRC : this.basketItemNoPhoto,
                    img = BX.create('IMG', {props: {src: url}});

                if (hasImage && allData.data.DETAIL_PICTURE_SRC_ORIGINAL && allData.data.DETAIL_PICTURE_SRC_ORIGINAL.length)
                    BX.bind(img, 'click', BX.delegate(function(e){
                        this.popupShow(e, allData.data.DETAIL_PICTURE_SRC_ORIGINAL);
                    }, this));
        ...
        createBasketItemHiddenColumn: function(column, allData)
        {
            ...
            else if (column.id == 'DETAIL_PICTURE' || column.id == 'PREVIEW_PICTURE')
            {
                var hasImage = allData.data.DETAIL_PICTURE_SRC && allData.data.DETAIL_PICTURE_SRC.length,
                    url = hasImage ? allData.data.DETAIL_PICTURE_SRC : this.basketItemNoPhoto,
                    img = BX.create('IMG', {props: {src: url}, style: {maxWidth: '50%'}});
        ...
        editActiveRegionBlock: function(activeNodeMode)
        {
            var node = activeNodeMode ? this.regionBlockNode : this.regionHiddenBlockNode,
                regionContent, regionNode, regionNodeCol, validationErrors;
            ...
            else
            {
                ...
                if (!regionContent)
                {
                    regionContent = this.getNewContainer(true);
				...
				buttons.push(
                    BX.create('A', {
                        props: {
                            href: 'javascript:BX.Sale.OrderAjaxComponent.editFadeRegionBlock();',
                            className: 'pull-left btn btn-default btn-md'
                        },
                        text: 'Отмена',
                    })
				...
				regionContent.appendChild(
					BX.create('DIV', {
						props: {className: 'row bx-soa-close'},
						children: [
							BX.create('DIV', {
								props: {className: 'bx-soa-close-btn col-xs-12'},
								children: buttons
							})
						]
					})
                ...
                {
                    validationErrors = this.isValidRegionBlock(true);
        ...
        editFadeRegionContent: function(node)
        {
            ...
            if (selectedPersonType && selectedPersonType.NAME)
            ...
            if (locationProperty && locationString.length)
                addedHtml += '<strong>' + BX.util.htmlspecialchars(locationProperty.NAME) + ':</strong> ' + locationString + ' <a href="javascript:BX.Sale.OrderAjaxComponent.editActiveRegionBlock(true);">изменить?</a> <br />';
        ...
        {
            var personTypeInput = this.regionBlockNode.querySelector('input[type=radio][name=PERSON_TYPE]:checked'),
                currentPersonType, personTypeId, i;
        ...
        editPaySystemItems: function(paySystemNode)
        {
            ...
            var paySystemItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 btn-group bx-soa-pp-item-container'}}),
                paySystemItemNode, i;
        ...
        createPaySystemItem: function(item)
        {
            var checked = item.CHECKED == 'Y',
                logotype = item.PSA_LOGOTIP && item.PSA_LOGOTIP.SRC && item.PSA_LOGOTIP.SRC.length ? item.PSA_LOGOTIP.SRC : this.paySystemDefaultLogo,
            ...
            label = BX.create('DIV', {
                ...
                children: [
                    ...
                    BX.create('DIV', {props: {className: 'bx-soa-pp-company-image'}, text: item.NAME})
                ]
            });
        ...
        editPaySystemInfo: function(paySystemNode)
        {
            ...
            if (currentPaySystem)
            {
                logotype = currentPaySystem.PSA_LOGOTIP && currentPaySystem.PSA_LOGOTIP.SRC && currentPaySystem.PSA_LOGOTIP.SRC.length
                    ? currentPaySystem.PSA_LOGOTIP.SRC
                    : this.paySystemDefaultLogo;
        ...
        getInnerPaySystem: function()
        {
            ...
            var accountOnly = this.params.ONLY_FULL_PAY_FROM_ACCOUNT && (this.params.ONLY_FULL_PAY_FROM_ACCOUNT == 'Y'),
                ...
                logotype = paySystem.LOGOTIP && paySystem.LOGOTIP.SRC && paySystem.LOGOTIP.SRC.length
                    ? paySystem.LOGOTIP.SRC
                    : this.paySystemDefaultLogo,
        ...
        editFadePaySystemContent: function(node)
        {
            ...
            if (this.isSelectedInnerPayment())
            {
                ...
                logotype = this.result.INNER_PAY_SYSTEM.LOGOTIP && this.result.INNER_PAY_SYSTEM.LOGOTIP.SRC && this.result.INNER_PAY_SYSTEM.LOGOTIP.SRC.length
                    ? this.result.INNER_PAY_SYSTEM.LOGOTIP.SRC
                    : this.paySystemDefaultLogo;
                addedHtml += '<img src="' + logotype + '" style="height:18px;" alt="">';
            ...
            {
                addedHtml += '<div class="bx-soa-pp-company-selected">';
                logotype = selectedPaySystem.PSA_LOGOTIP && selectedPaySystem.PSA_LOGOTIP.SRC && selectedPaySystem.PSA_LOGOTIP.SRC.length
                    ? selectedPaySystem.PSA_LOGOTIP.SRC
                    : this.paySystemDefaultLogo;
                addedHtml += '<img src="' + logotype + '" style="height:18px;" alt="">';
        ...
        editDeliveryItems: function(deliveryNode)
        {
            ...
            var deliveryItemsContainer = BX.create('DIV', {props: {className: 'confirm-tab-shopcart bx-soa-pp-item-container'}}),
                deliveryItemNode, k;
        ...
        editDeliveryInfo: function(deliveryNode)
        {
            ...
            var deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-pp-desc-container'}}),
                currentDelivery, logotype, name,
                subTitle, label, title, price, period,
                clear, infoList, extraServices, extraServicesNode;

            BX.cleanNode(deliveryInfoContainer);
            currentDelivery = this.getSelectedDelivery();
            logotype = currentDelivery.LOGOTIP && currentDelivery.LOGOTIP.length ? currentDelivery.LOGOTIP : this.deliveryDefaultLogo;
                ...
                price = BX.create('LI', {
                    children: [
                        ...
                        BX.create('DIV', {
                            ...
                            children: typeof currentDelivery.DELIVERY_DISCOUNT_PRICE !== 'undefined'
                                    && parseFloat(currentDelivery.DELIVERY_DISCOUNT_PRICE) != parseFloat(currentDelivery.PRICE)
                                ? [
                                    currentDelivery.DELIVERY_DISCOUNT_PRICE_FORMATED,
                                    BX.create('BR'),
                                    BX.create('SPAN', {
                                        props: {className: 'bx-price-old'},
                                        text: currentDelivery.PRICE_FORMATED
                                    })
        ...
        {		
            var checked = item.CHECKED == 'Y',
                logotype = item.LOGOTIP && item.LOGOTIP.length ? item.LOGOTIP : this.deliveryDefaultLogo,
            ...
            itemNode = BX.create('DIV', {
                props: {className: 'bx-soa-pp-company confirm-tab-item'},
                children: [label, title],
                events: {click: BX.proxy(this.selectDelivery, this)}
            });
            checked && BX.addClass(itemNode, 'bx-selected active confirm-active_tab');
			
			/*itemNode = BX.create('button', {
			...
			/*if(deliveryId==33 && checked) 
			{
				//("#iml-pickup").show();
				IPOLIML_pvz.selectPVZ('iml-pickup');
			}
        ...
        editFadeDeliveryContent: function(node)
        {
            var selectedDelivery = this.getSelectedDelivery(),
                ...
                extraService, logotype, arNodes, i;
            ...
            {
                logotype = selectedDelivery.LOGOTIP && selectedDelivery.LOGOTIP.length ? selectedDelivery.LOGOTIP : this.deliveryDefaultLogo;
                arNodes = [
                    BX.create('IMG', {props: {src: logotype, alt: ''}, style: {height: '18px'}}),
                ...
                node.appendChild(
                    BX.create('DIV', {
                        ...
                        children: typeof selectedDelivery.DELIVERY_DISCOUNT_PRICE !== 'undefined'
                            && parseFloat(selectedDelivery.DELIVERY_DISCOUNT_PRICE) != parseFloat(selectedDelivery.PRICE)
                                ? [
                                    selectedDelivery.DELIVERY_DISCOUNT_PRICE_FORMATED,
                                    BX.create('BR'),
                                    BX.create('SPAN', {props: {className: 'bx-price-old'}, text: selectedDelivery.PRICE_FORMATED})
                                ] : [selectedDelivery.PRICE_FORMATED]
                    })
                );
            }
            else
        ...
        activatePickUp: function(deliveryName)
        {
            ...
            //this.pickUpBlockNode.querySelector('h2.bx-soa-section-title').innerHTML = deliveryName;
        ...
        {
         /*   var selectedPickUp = this.getSelectedPickUp(), html;
...
*/
        ...
        createPickUpItem: function(currentStore, options)
        {
            ...
            var imgSrc = currentStore.IMAGE_ID && currentStore.IMAGE_ID.SRC ? currentStore.IMAGE_ID.SRC : this.storeDefaultLogo,
                imgClassName = 'bx-soa-pickup-l-item-detail',
                buttonClassName = 'bx-soa-pickup-l-item-btn',
                imgNode, html, storeNode;

            if (this.params.SHOW_STORES_IMAGES == 'Y')
                imgNode = BX.create('IMG', {props: {className: 'bx-soa-pickup-l-item-img', src: imgSrc, alt: ''}});
            else
            {
        ...
        editPropsBlock: function(active)
        {
			...
			//$( "input[name=ORDER_PROP_27]" ).prop("readonly", true);
			
			$( "input[name=ORDER_PROP_7]" ).datepicker(
				{
					showOn: "both",
					//buttonImage: "/bitrix/templates/new/img/calendar.png",
			...
			if($("input[name=ORDER_PROP_7]").val()=="" || $("input[name=ORDER_PROP_7]").val()==null) 
			{
				$("input[name=ORDER_PROP_7]").datepicker('setDate', new Date($("#start_date").val()));
				$("input[name=ORDER_PROP_7]").attr("readonly", "readonly");
			}
			
			$("#phone").val($("#soa-property-3").val());
			$('#phone').wmPhoneFormatSuggest();						
        ...
        editActivePropsBlock: function(activeNodeMode)
        {
            ...
            {		
                ...
                if (!propsContent)
                {
                    propsContent = this.getNewContainer(true);
				...
				$(".hidden-prop").before('<div class="soa-property-container"><input class="form-control bx-soa-customer-input bx-ios-fix" placeholder="Телефон" type="text" maxlength="250" size="" value="" id="phone"></div>');
            }
        },
        ...
        editPropsItems: function(propsNode)
        {
            ...
            var propsItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-customer'}}),
                group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator, groupPropertID, recipient_label, groupName, parentProp;
            ...
            while (group = groupIterator())
            {
                ...
                while (property = propsIterator())
                {
                    if (this.deliveryLocationInfo.loc == property.getId()
						...
						|| property.getId() == 'orderDescription')
                        continue;
						
					groupPropertID = property.getGroupId();

					 if(groupName != this.propGroups[groupPropertID]){
						groupName = this.propGroups[groupPropertID];
						parentProp = BX.create('DIV', {
						   props: {className: 'bx-soa-group col-xs-12', id: 'group_'+groupPropertID},
						   children: [
							  BX.create('DIV', {
								 props: {className: 'bx-soa-group-name'}, text: groupName,
							  }),
						   ]
						});
					 }
                    this.getPropertyRowNode(property, parentProp, false);
                }
				propsNode.appendChild(parentProp);
            }
        },

        getPropertyRowNode: function(property, propsItemsContainer, disabled)
        {
            var propsItemNode = BX.create('DIV', {props: {className: "form-group bx-soa-customer-field class-prop-" + property.getId()}}),
                labelTextHtml = '',
        ...
        {
            var propRow, propNodes, locationString, currentLocation, insertedLoc, propContainer, i, k;
                    ...
                    if (propRow)
                    {
                        ...
                        for (i = 0; i < propNodes.length; i++)
                        {
                            ...
                            propsItemNode.appendChild(
                                BX.create('DIV', {text: locationString.length ? locationString : BX.message('SOA_NOT_SELECTED')})
                            )
                        }
                    }
                }
        ...
        insertFileProperty: function(property, propsItemNode, disabled)
        {
            ...
            if (disabled)
            {
                ...
                if (prop)
                {
                    ...
                    propsItemNode.appendChild(BX.create('DIV', {html: this.valuesToString(values)}));
                }
            }
            else
            {
        ...
        insertStringProperty: function(property, propsItemNode, disabled)
        {
            ...
            else
            {
				if(property.getId()==3)
				{
					propContainer = BX.create('DIV', {props: {className: 'soa-property-container hidden-prop'}});
        ...
        isValidRegionBlock: function(excludeLocation)
        {
            if (!this.options.propertyValidation)
                return [];
        ...
        validateFile: function(inputs, arProperty, fieldName)
        {
            ...
            if (arProperty.REQUIRED == 'Y' && files.length == 0)
                errors.push(field + ' ' + BX.message('SOA_REQUIRED'));
            else
            {
        ...
        createTotalUnit: function(name, value, params)
        {
            ...
            return BX.create('DIV', {
                props: {className: (!!params.total ? 'sec-cart__info-total clearfix' : 'sec-cart__info-field sec-cart__info-field--sale clearfix')},
                children: [
        ...
        totalBlockScrollCheck: function()
        {
            ...
            if (ghostBottom - this.totalBlockNode.offsetHeight * 0.7 < scrollTop)
	...
	$.widget("ui.wmPhoneFormatSuggest", {
		_create: function() {
			var $element = this.$element = $(this.element);
			//console.log($element);
			if ($element.data('wmPhoneFormatSuggestInstance') != null) {
				return false;
			}
			// Чохран§ем ссылку на экземпл§р
			$element.data('wmPhoneFormatSuggestInstance', this);
			// Фстанавливаем обработчики
			$element.bind('change keypress keyup click blur focus', this, this.elementChangeHandler);
			// Цегистрируем коллбэк дл§ валидатора
			$element.
					addClass('validatable').
					data('validator', this.validatePhone).
					data('invalid-message', 'Неверный формат телефонного номера');			
		},
		
		options:{
			renderSuggest:function ($element, text) {
				if (this.isInitialized === undefined) {
					...
					$element.qtip({
						...
						position:$element.wmPhoneFormatSuggest('option', 'qtipPositionOptions'),
						show:{
							event:'none'
						},
						hide:{
						...
						style:{
							classes:'qtip-rounded qtip-shadow cart-format'
						}
					});
		...
		elementChangeHandler:function (e) {
			var instance = e.data;
			if (e.type == 'focus') {
				window.setTimeout(function () {
					var val = instance.$element.val();
					instance.$element.val('').val(val);
				}, 0);
			}
			// Дистим строку
			var userInput = instance.$element.val();
			var number = new phoneNumber(userInput, instance.options.phoneWithExtension);
			if (this.lastClearedString != number.clearedString) {
				this.lastClearedString = number.clearedString;
			}
			// Њоказываем формат
			if (userInput.length) {
				// Њоказываем сообщение
				var text = 'Вы написали: ' + userInput + '<br />';
				if (number.errorMessage) {
					text += number.errorMessage;
					$("#soa-property-3").val("");
				}
				else if (number.clearedString == '') {
					text += 'Мы ничего не поняли, наверное это не номер телефона';
					$("#soa-property-3").val("");
				}
				else {
					text += 'Мы поняли: ' + number.formatedString; 
					
					var phone = number.formatedString;
					phone = phone.replace(/\+7/g,"8");
					phone = phone.replace(/[^0-9]/g,"");
					$("#soa-property-3").val(phone);
				}
				instance.options.renderSuggest(instance.$element, text);
			}
			else {
				instance.options.renderSuggest(instance.$element, "");
			}
		},
		format:function (clearedString) {
			var number = new phoneNumber(clearedString, this.options.phoneWithExtension);
			return number.formatedString;
		},
		clear:function (userString) {
			var number = new phoneNumber(userString, this.options.phoneWithExtension);
			return number.clearedString;
		},
		validatePhone:function (value) {
			var number = new phoneNumber(value, this.options.phoneWithExtension);
			return 'noticed' == number.status || 'good' == number.status;
		}
	});
	//@TODO перевести на ф-и setError, setNotice
	var phoneNumber = function(originalString, withExtension){
		this.minEnternalNumberLength = 2;
		this.maxEnternalNumberLength = 5;
		this.recomendedEnternalNumberLength = [3];
		this.withExtension = false;
		if(arguments.length === 2)
		{
			this.withExtension = withExtension;
		}
		this.originalString = originalString; //содержимое инпута как есть
		this.clearedString  = ''; //строка, очищенна§ от лишних символов, 8 заменена на +7, 00 на + (так хранитс§ в бд)
		this.formatedString = ''; //строка, отформатированна§ дл§ показа пользователю
		...
		/**
		 * оцененное состо§ние телефонного номера, возможные состо§ни§:
		 *  notFinished - телефон пока не относитс§ ни к верным, ни к ошибочным - нужно продолжение ввода
		 *  good - телефон проходит валидацию, можно сохран§ть в системе
		 *  bad  - телефон точно не подходит нам
		 *  noticed - телефон принимаем, но пользователю нужно вывести предупреждение, что возможно он описалс§
		 */
		this.status = 'bad'; //если отвалились, не помен§в статус - §вно что-то не так, пропускать нельз§ :)
		...
		{
			if(this.clearedString.length > 20){
				this.status = 'bad';
				this.errorMessage = 'Телефон не может быть настолько длинным';
				return false;
			}
			if(this.clearedString == '+'){
				this.status = 'notFinished';
				return true;
			}

			var info =  phoneRules.getInfo(this.clearedString.replace(/^\+/g,""));

			if(info['status'] == 'bad'){
				this.status = 'bad';
				this.errorMessage = 'Телефон не может быть из зарезервированной зоны';
				return false;
			}

			if(info['status'] == 'noticed'){
				this.status = 'bad';
				this.errorMessage = 'Не удалось найти зону телефона';
				return false;
			}

			if(!info['regionLength'] || !info['numberLength']){
				//по идее, этого кейса возникать не должно, но если конфиг заполнен неверно - может и возникнуть
				return false;
			}

			this.parsePhoneNumber(info);
			this.formatPhoneNumber(info);
		};

		this.parsePhoneNumber = function(info){
			var phone = this.clearedString.replace(/^\+/g,"");
			this.status = 'notFinished';

			this.zone =  phone.substring(0, info['zoneLength']);
			if(this.zone.length < info['zoneLength']){
				return false;
			}

			this.region =  phone.substring(info['zoneLength'], (info['zoneLength'] + info['regionLength']));
			if(this.region.length < info['regionLength']){
				return false;
			}

			this.mainNumber =  phone.substring((info['zoneLength'] + info['regionLength']), (info['zoneLength'] + info['regionLength'] + info['numberLength']));
			if(this.mainNumber.length < info['numberLength']){
				return false;
			}
			this.status = 'good';

			this.enternalNumber = phone.substring((info['zoneLength'] + info['regionLength'] + info['numberLength']));
			if(this.enternalNumber.length == 0){
				//?обавочного номера нет, его валидировать не нужно
				return true;
			}
			if(!this.withExtension){
				this.status = 'bad';
				this.errorMessage = 'Телефон должен быть указан без добавочного номера';
				return false;
			}
			if(this.enternalNumber.length > this.maxEnternalNumberLength){
				this.status = 'bad';
				this.errorMessage = 'Добавочный номер не может превышать ' + this.maxEnternalNumberLength + ' символов';
				return false;
			}
			if(this.enternalNumber.length < this.minEnternalNumberLength){
				this.status = 'bad';
				this.errorMessage = 'Добавочный номер не может быть меньше ' + this.minEnternalNumberLength + ' символов';
				return false;
			}

			for(i in this.recomendedEnternalNumberLength){
				if(this.recomendedEnternalNumberLength[i] == this.enternalNumber.length){
					//длина добавочного номера попала в список рекомендуемых
					return true;
				}
			}

			this.status = 'noticed';
			this.noticeMessage = 'Обратите внимание на добавочный номер, возможно, в нем ђы допустили ошибку';

		};

		this.formatPhoneNumber = function(info){
			this.formatedString = '';
			if(this.zone.length > 0){
				this.formatedString += '+' + this.zone;
			}
			else{
				return false;
			}
			...
			if(this.region.length > 0){
				this.formatedString += ' (' + this.region + ')';
			}
			else{
				this.formatedString += '';
				return false;
			...
			if(this.mainNumber.length > 0){
				this.formatedString += ' ' + this.formatMainNumber(info);
			}

			if(info['numberLength'] > this.mainNumber.length){
				this.formatedString += '';
				return false;
			}

			if(this.enternalNumber.length > 0){
				this.formatedString += ', доб. ' + this.enternalNumber;
			}
		};

		this.formatMainNumber = function(info){
			if(info['numberLength'] != 7 || this.mainNumber.length < 3){
				return this.mainNumber;
			}
			var toReturn = this.mainNumber.substring(0,3) + '-' + this.mainNumber.substring(3,5);

			if(this.mainNumber.length < 5){
				return toReturn;
			}

			toReturn = toReturn + '-' + this.mainNumber.substring(5,7);

			return toReturn;
		};

		this.clearString = function(){
			var clearedString = this.originalString.replace(/[^+0-9]/g,"");
			clearedString = clearedString.replace(/^8(.*)/g,"+7$1");
			clearedString = clearedString.replace(/^00(.*)/g,"+$1");
			if(clearedString.substring(0,1) == '+'){
				var start = '+';
			}
			else{
				var start = '';
			}
			this.clearedString = start + clearedString.replace(/\+/g,"");
		};
	...
	var phoneRules = {
		/**
		 * правила парсинга, тут и зоны и регионы. ?сли объ§влены данные помимо числовых - значит
		 * это начало нового блока (зоны, региона и тд). Чтатус может быть либо good, либо bad
		 * статус good - если эта зона или регион предназначена дл§ пользовани§ физ. лицами
		 * status bad - если эта зона точно не может быть использована (она зарезервирована и тд)
		 *
		 * 'regionLength' и 'numberLength' - об§зательные пол§, должны идти в зонах, вместе
		 */
		Rules : {
			'7' : {
				'regionLength' : 3,
		...
		getInfo : function(phoneNum){
			var phoneLength = phoneNum.length;
			var info = {'status' : 'noticed'};
			var currentRule = phoneRules.Rules;

			for(var i=0; i<phoneLength; i++){
				var Character = phoneNum.substr(i,1);
				if(typeof(currentRule[Character]) == 'undefined'){
					//дальше описание отсутствует, смотрим, что накопили раньше
					break;
				}
				//тут смотрим, что же нам попалось :)
				currentRule = currentRule[Character];
				if(currentRule['regionLength'] && currentRule['numberLength']){
					info['zoneLength'] = (i+1);
					info['regionLength'] = currentRule['regionLength'];
					info['numberLength'] = currentRule['numberLength'];
				}
				if(currentRule['country']){
					info['country'] = currentRule['country'];
				}
				if(currentRule['region']){
					info['region'] = currentRule['region'];
				}
				if(currentRule['status']){
					info['status'] = currentRule['status'];
				}
				if(info['status'] == 'bad'){
					break;
				}
